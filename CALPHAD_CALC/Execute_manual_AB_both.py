...
plot_descriptor = "AB_both_manual"
...
for num in range(len(file_names_BCC)):
    for xps in range(len(x_Tis)):
        ...
        solids =[[["FE"],["TI"]], [["FE"],["TI"]]]  # Define solids involved in the structure
        multiplicities = [multiplicitiess_BCC[num], multiplicitiess_FCC[num]]  # Define the multiplicity of sites in the structure
        x_NB = 1
        site_fractions = [[[x_NB],[1]], [[x_NB], [1]]]  # Initial site fractions for hydrogen
        color = ["blue"]
        x_Ti = x_Tis[xps]
