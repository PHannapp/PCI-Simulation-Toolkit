...
plot_descriptor = "AB_beta_manual_parity"
...
for num in range(len(file_names)):
    for xps in range(len(x_Tis)):
        ...
        for i in range(1):
            ...
            p_exp = ([line[1] for line in exp[0]])
            x_exp = [line[0] for line in exp[0]]



            # Interpolate p_plot to match x_exp values
            interp_func = interp1d(x_plot, p_plot, kind='linear', fill_value="extrapolate")
            p_plot_interp = interp_func(x_exp)
            # Find common axis limits
            min_val = min(min(p_exp), min(p_plot_interp))
            max_val = max(max(p_exp), max(p_plot_interp))
            parity = np.linspace(min_val / 2, max_val * 2, 10)
            # Create the parity plot
            plt.plot(parity, parity, 'k--', label="Parity Line")  # Parity line
            plt.plot(p_exp, p_plot_interp, 'o', label="Model", color="blue")
            ...
            plt.xlim(min(parity), max(parity))
            plt.ylim(min(parity), max(parity))
...
#    plt.semilogy([line[0] for line in exp[i]], [line[1] for line in exp[i]], 'o', color = exp_colors[i], linestyle='-', label=exp_names[i])
