...
def GAL_V(T):
 return GBCCAL(T) 
def GAL_H(T):
 return 1203150 +733.3*T + GHSERAL(T) + 3*GHSERH2(T)
# Co-H, Kim, 2012
def GCO_V(T):
 return GBCCCO(T) 
def GCO_H(T):
 return 1245550 +733.3*T + GHSERCO(T) + 3*GHSERH2(T)
# Cr-H, Joubert, 2022
def GCR_V(T):
 return GHSERCR(T) 
def GCR_H(T):
 return GHSERCR(T) + 1.5*GHSERH2(T) + 1.2065e5 + 1.9620e2 * T
# Fe-H, Zinkevich, 2002
def GFE_V(T):
 return GHSERFE(T)
def GFE_H(T):
...
def GMN_V(T):
 return GBCCMN(T)
def GMN_H(T):
 return GBCCMN(T) + 1.5*GHSERH2(T) + 4*(13456.995 + 36.5144*T)
# Mo-H, TC + DFT?
def GMO_V(T):
 return GHSERMO(T) 
# Nb-H, Dottor, 2023
def GNB_V(T):
 return GHSERNB(T)
def GNB_H(T):
 return GHSERNB(T) + 1.5*GHSERH2(T) +199994.208 + 196.2*T
# Ni-H, Bourgeois, 2015, DFT?
def GNI_V(T):
 return GBCCNI(T)
# Sc-H, TC + DFT?
def GSC_V(T):
...
def GTI_V(T):
 return GBCCTI(T)
def GTI_H(T):
 return GBCCTI(T) + 0.75*GHSERH2(T) -95951.4 + 95.036*T
# V-H, Ukita, 2008
def GVA_V(T):
 return GHSERV(T) 
def GVA_H(T):
 return GHSERV(T) + 3*GHSERH2(T) + 7*(78300 + 75.2*T)
# Zr-H, Dottor, 2022
def GZR_V(T):
 return GBCCZR(T)
def GZR_H(T):
 return GBCCZR(T) + 1.5*GHSERH2(T) +44853
...
# Cr-H, Joubert, 2022
def LCR_HV_0(T):
 return -3.7280e3 - 3.8378e1 * T
# Fe-H, Zinkevich, 2002, nothing?
# Hf-H, TC + DFT?
# Mg-H, TC + DFT?
# Mn-H, Liang, 2018, nothing?
# Mo-H, TC + DFT?
# Nb-H, Dottor, 2023
def LNB_HV_0(T):
 return -563483
...
def LNB_HV_3(T):
 return 52413
# Ni-H, Bourgeois, 2015, TC + DFT?
# Sc-H, TC + DFT?
# Ta-H, TC + DFT?
# Ti-H, Wang, 2010
def LTI_HV_0(T):
 return 2970.28 - 6.5847*T
def LTI_HV_1(T):
 return -7380.82 - 0.3547*T
# V-H, Ukita, 2008
def LVA_HV_0(T):
 return 7*(-116700 - 0.3*T)
def LVA_HV_1(T):
 return 7*(75400 - 0.6*T)
def LVA_HV_2(T):
 return 7*(90500 - 17.6*T)
# Zr-H, Dottor, 2022
def LZR_HV_0(T):
 return -474971 + 359*T
def LZR_HV_1(T):
