...
def GHF_V(T):
 return GBCCHF(T) 
#def GHF_H(T):
# return GHSERHF(T) + 0.5*GHSERH2(T) + !
# Mg-H, from xxx
def GMG_V(T):
 return GBCCMG(T) 
#def GMG_H(T):
# return GHSERMG(T) + 0.5*GHSERH2(T) + !
...
def GMO_V(T):
 return GHSERMO(T) 
#def GMO_H(T):
# return GHSERMO(T) + 0.5*GHSERH2(T) + !
# Nb-H, Dottor, 2023, trimmed
def GNB_V(T):
 return GHSERNB(T)
def GNB_H(T):
 return GHSERNB(T) + 0.5*GHSERH2(T) -37171 + 49.5*T
# Ni-H, Bourgeois, 2015, DFT?, from xxx
def GNI_V(T):
 return GBCCNI(T)
#def GNI_H(T):
# return GHSERNI(T) + 0.5*GHSERH2(T) + !
# Sc-H, from xxx
def GSC_V(T):
 return GBCCSC(T) 
#def GSC_H(T):
# return GHSERSC(T) + 0.5*GHSERH2(T) + !
# Ta-H, from xxx
def GTA_V(T):
 return GHSERTA(T)
#def GTA_H(T):
# return GHSERTA(T) + 0.5*GHSERH2(T) + !
