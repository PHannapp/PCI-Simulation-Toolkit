...
# Endmembers -------------------------------------------------------------------------------
# Al-H, Qiu, 2004, extend
def GAL_V(T):
 return GHSERAL(T) 
def GAL_H(T):
 return GHSERAL(T) + GHSERH2(T) + !
# Co-H, Kim, 2012, extend
def GCO_V(T):
 return GFCCCO(T) 
def GCO_H(T):
 return GHSERCO(T) + GHSERH2(T) + !
# Cr-H, Joubert, 2022, extend
def GCR_V(T):
 return GFCCCR(T) 
def GCR_H(T):
 return GHSERCR(T) + GHSERH2(T) + !
# Fe-H, Zinkevich, 2002, extend
def GFE_V(T):
 return GFCCFE(T)
def GFE_H(T):
 return GFCCFE(T) + GHSERH2(T) + !
# Hf-H, DFT?
def GHF_V(T):
 return GHSERHF(T) 
# Mg-H, DFT?
def GMG_V(T):
 return GFCCMG(T) 
# Mn-H, Liang, 2018, extend
def GMN_V(T):
 return GFCCMN(T)
def GMN_H(T):
 return GFCCMN(T) + GHSERH2(T) + !
# Mo-H, DFT?
def GMO_V(T):
 return GFCCMO(T) 
# Nb-H, Dottor, 2023
def GNB_V(T):
 return GFCCNB(T)
def GNB_H(T):
 return GHSERNB(T) + GHSERH2(T) -63103.503 + 128.56*T
# Ni-H, Bourgeois, 2015, extend
def GNI_V(T):
 return GHSERNI(T)
def GNI_H(T):
 return GHSERNI(T) + GHSERH2(T) + !
# Sc-H, DFT?
def GSC_V(T):
 return GFCCSC(T) 
# Ta-H, DFT?
